...
impl<'a, 'b> Parser<'a, 'b> {
    ...
    fn parse_regs(&mut self) -> Option<Vec<node::Reg>> {
        ...
        // TODO(wannes): HOWEVER overlapping registers produce invalid code due to the C struct packing
        /*
          for example:
          0x0 => reg16 bla; will be at offset 0x0
          0x0 => reg16 alb; will be at offset 0x2 (!) due to the C struct packing
          0x0 => reg16 lab; will be at offset 0x4 (!)
          0x4 => reg16 bal; will be at offset 0x6 (!)
        */
        ...
        for (r1, r2) in regs.iter().zip(regs.iter().skip(1)) {
            if r2.offset <= r1.last_byte() {
                self.sess.span_diagnostic.span_err(
                    r1.name.span,
                    format!("The byte range of register ({} to {})", r1.offset, r1.last_byte())
                        .as_str());
                self.sess.span_diagnostic.span_err(
                    r2.name.span,
                    format!("overlaps with the range of this register ({} to {}). \
                    This is currently unsupported due to the way the registers are memory \
                    mapped with `repr(C)`", r2.offset, r2.last_byte()).as_str());
                failed = true;
            }
        }
                ...
                match self.token.clone() {
                    ref t @ token::Ident(_) => {
                        match pprust::token_to_string(t) {
                            ...
                            ref s if s.eq(&"wo") => {
                                self.bump();
                                node::Access::WriteOnly
                            }
                            ref s if s.eq(&"set_to_clear") => {
                                self.bump();
